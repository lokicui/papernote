...
Many approaches are proposed to generate the timeline summarization, but ignore the life circle of each topic. In this paper, aging theory is leveraged to compute the sentence feature in the first step, and then train the classification model with the SMOTEBoost technology. 
...
The other challenge is how to deal with the labeled dataset which is common problem in the field of machine learning.
In each topic, only a few sentences will be labled as the summarization sentence, that is, the data set is imbalanced. 
This situation causes a problem that the training model will prefer the normal sentences. 
